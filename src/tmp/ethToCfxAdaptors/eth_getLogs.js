...
async function inputAdaptor(params) {
  if (params.length > 0) {
    ...
    if (params[0].address) {
      if (_.isArray(params[0].address)) {
        params[0].address = params[0].address.map(a => format.formatAddress(a, this.cfx.networkId));
      } else {
        params[0].address = format.formatAddress(params[0].address, this.cfx.networkId);
      }
    }
  }
}
